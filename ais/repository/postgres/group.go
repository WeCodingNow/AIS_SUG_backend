...
func (g *repoGroup) Fill(scannable Scannable) {
...
func (c repoGroup) GetDescription() ModelDescription {
		...
		Dependencies: []ModelDependency{
			{
				...
				ModelMaker:      func() RepoModel { return NewRepoCathedra() },
			},
			{
...
func (r *DBAisRepository) GetGroup(ctx context.Context, id int) (*models.Group, error) {
	group := NewRepoGroup()
	filler, err := MakeFiller(ctx, r.db, groupFields, groupTable, &id)
...
func (r *DBAisRepository) GetAllGroups(ctx context.Context) ([]*models.Group, error) {
	...
	filler, err := MakeFiller(ctx, r.db, groupFields, groupTable, nil)
